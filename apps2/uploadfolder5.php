...
$location ="folder";
$fileName = $_FILES["myfile".$w]["name"]; // The file name
$fileTmpLoc = $_FILES["myfile".$w]["tmp_name"]; // File in the PHP tmp folder
$fileType = $_FILES["myfile".$w]["type"];
if($w==1)
{
$myfiletype="png";
}
...
$fileSize = $_FILES["myfile".$w]["size"]; // File size in bytes
$fileErrorMsg = $_FILES["myfile".$w]["error"]; // 0 for false... and 1 for true
...
$sql7= "DELETE FROM `AAtask` WHERE `user`='$encUser' and `name`='$ref2' and `filename`='$filename'";
$result7 = mysqli_query($con, $sql7);

$sql3= "INSERT INTO `AAtask` (`id`, `name`, `approve`, `filename`, `filecodename`, `extention`, `type`, `user`, `ext1`, `ext2`, `ext3`) VALUES (NULL, '$ref2', 'pending', '$filename', '$codename', '$extn', '$filename', '$encUser', '', '', '')";
