<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">
	...
	*/
    ...
    $req1xa = "SELECT * FROM `AAissue` WHERE `user`='$encUser' and `emp`='$empid' and `project`='$project' GROUP BY `token` ORDER BY `date` DESC";
   ...
   echo '<tr>
	...
	$req1xa = "SELECT * FROM `AAissue` WHERE `user`='$encUser' and `emp`='$empid' and `project`='$project' GROUP BY `token` ORDER BY `date` ASC";
	$reqRxa = mysqli_query($con, $req1xa);
	while ($resutAa =mysqli_fetch_assoc($reqRxa))
	{
	$xwing = $resutAa["wing"];
