...
$findpass = "SELECT * FROM `AAuser` WHERE `id`='$uid'";
   $resultfindpass= mysqli_query($con, $findpass);
   $rowfindpass = mysqli_fetch_assoc($resultfindpass);
   $fetchpass =$rowfind["password"];
   if($fetchpass==$adminpassword)
{
$queryA="UPDATE `AAuser` SET `password` = '$password' WHERE `AAuser`.`id` = '$uid'";
   ...
   $Adescription ="You have password changes";
   $Atitle = " changes";
   $Aref1 ="1";
   $Aref2 ="1";
   $Aref3 ="";
