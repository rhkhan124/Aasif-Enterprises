...
$location ="folder";
$fileName = $_FILES["myfile".$s]["name"]; // The file name
$fileTmpLoc = $_FILES["myfile".$s]["tmp_name"]; // File in the PHP tmp folder
$fileType = $_FILES["myfile".$s]["type"];
$myfiletype = pathinfo($fileName, PATHINFO_EXTENSION); // The type of file it is
$fileSize = $_FILES["myfile".$s]["size"]; // File size in bytes
$fileErrorMsg = $_FILES["myfile".$s]["error"]; // 0 for false... and 1 for true
