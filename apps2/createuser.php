...
if($Uemail==$email)
{
  ...
  $queryA="INSERT INTO `AAuser`(`name`,`Main`, `company`, `mobile`, `email`, `role`, `password`, `verified`, `image`, `date`, `primium`, `lastactive`, `ext2`, `ext3`, `ext4`,`project`,`permission`,`type`,`employee`) VALUES ('$name','$decUser','','$phone','$email','$role2','$password',0,0,'$date',0,'$datetime','active','','','$project','$permission','','')";
   ...
   $find = "SELECT * FROM `AAuser` WHERE `mobile`='$phone'";
   $resultfind = mysqli_query($con, $find);
   $rowfind = mysqli_fetch_assoc($resultfind);
   $focus =$rowfind["id"];
   
   $Adescription ="created user ";
   $Atitle = "desination ".$role2;
   $Aref1 ="1";
   $Aref2 ="1";
   $Aref3 ="";
   $Aref4 ="";
