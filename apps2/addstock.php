...
$find2 = "SELECT AAproject.name pname, AAitemname.name item FROM AAproject, AAitemname WHERE AAproject.id='$project' and AAitemname.id='$itemx'";
  ...
  $alertdata =$alertdata.($itemname2." ".$qty." ".$unit." amount  ".$amount."<br>");

$req1x = "SELECT * FROM `AAstock` WHERE `user`='$encUser' and `id`='$idx'";
$reqRx = mysqli_query($con, $req1x);
if (mysqli_num_rows($reqRx ) > 0)
{
$queryBd="UPDATE `AAstock` SET `qty` = '$qty', `unit` = '$unit', `dealer` = '$dealer', `date` = '$date2', `challan` = '$challan', `amount` = '$amount' WHERE `AAstock`.`id` = '$idx';";
 $resultd = mysqli_query($con, $queryBd);
}
else
{

$req1 = "SELECT * FROM `AAstock` WHERE `user`='$encUser' and `project`='$project' and `date`='$date2' and `challan`='$challan' and `item`='$itemx' and `qty`='$qty'";
$reqR = mysqli_query($con, $req1);
if (mysqli_num_rows($reqR ) > 0)
{
$xyz =0;
...
{
  $queryB="INSERT INTO `AAstock` (`item`, `qty`, `unit`, `dealer`, `date`, `challan`, `amount`, `project`, `user`, `addedby`, `ext1`, `ext2`, `ext3`) VALUES ('$itemx', '$qty', '$unit', '$dealer', '$date2', '$challan', '$amount', '$project', '$encUser', '$encId', '', '', '');";
  $result = mysqli_query($con, $queryB);
}
}
}
$queryBm="DELETE FROM `AAstock` WHERE `qty`=''";
...
$find = "SELECT AAproject.name pname FROM AAproject WHERE AAproject.id='$project'";
  $resultfind = mysqli_query($con, $find);
  $rowfind = mysqli_fetch_assoc($resultfind);
  $pname =$rowfind["pname"];
  
  
  $Adescription =$pname." stock updated";
