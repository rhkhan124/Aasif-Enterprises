...
$sql16 = "SELECT SUM(amount) as salary FROM `AASalary` WHERE `name`='$empid'";
...
$sql14= "SELECT * FROM `AAemployees` WHERE `user`='$encUser' and `ext1`='$empid'";
...
while ($row14 = mysqli_fetch_assoc($result14))
{
 ...
 $myObj2->role  =$row14['role'];
   ...
   $myJSON2B = json_encode($myObj2);
   $myJSON2 =$myJSON2.$myJSON2B."#";
 }
...
// for employee user and password show
$check2=0;
$permissionA2 = explode(",",$permissionArray);
for ($x = 0; $x <= count($permissionA2); $x++) 
{
if($permissionA2[$x]==9)
{
$check2 =1;
