...
{
$myreq ="AAPaid";
$sql8 = "SELECT * FROM `$myreq` WHERE `user`='$encUser' and `name`='$empid' and `ext1`='true' and `date` >='$fdate' and `date`<='$ldate' ORDER BY `date` DESC";
...
$sql8= "SELECT * FROM `$myreq` WHERE `user`='$encUser' and `ext1`='true' and `name`='$empid' and `date` >='$fdate' and `date`<='$ldate' ORDER BY `date` DESC";
$result8 = mysqli_query($con, $sql8);
while ($row8 = mysqli_fetch_assoc($result8))
{
$empid =$row8['name'];
