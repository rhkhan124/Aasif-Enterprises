...
if($type=="main")
{
    ...
    $req1xabf = "SELECT SUM(amount) as allstock FROM `AAstock` WHERE `user`='$encUser' and `project`='$project'";
    $reqRxabf = mysqli_query($con, $req1xabf);
    $resutAabf =mysqli_fetch_assoc($reqRxabf);
    
    $allstockag = $resutAabf["allstock"];
    
    $req1xabfd = "SELECT SUM(amount) as paid FROM `AAPaid` WHERE `user`='$encUser' and `project`='$project'";
    $reqRxabfd = mysqli_query($con, $req1xabfd);
    $resutAabfd =mysqli_fetch_assoc($reqRxabfd);
    
    $allstockagd = $resutAabfd["paid"];
    
    $req1xabfp = "SELECT SUM(amount) as ss FROM `AASalary` WHERE `user`='$encUser' and `project`='$project'";
    $reqRxabfp = mysqli_query($con, $req1xabfp);
    $resutAabfp =mysqli_fetch_assoc($reqRxabfp);
    
    $allstockagp = $resutAabfp["ss"];

    $unpaid =$allstockagp-$allstockagd;
echo '<div class="mycontainer box" style="margin:3%">
<table class="F12" style="width:90%;margin-left:5%;font-weight:bold;font-family:times;">
<tr class="h">
<td class="F18" colspan="2"  style="text-align:center;">Project Analysis</td>
</tr>
<tr class="h">
<td style="" >Total Material & Utilities Cost</td>
<td class="r" >'.$allstockag.'/-</td>
</tr>
<tr class="h" >
<td style="" >Total Paid Salary</td>
<td class="r" >'.$allstockagd.'/-</td>
</tr>
<tr class="h">
<td style="" >Total Salary</td>
<td class="r" >'.$allstockagp.'/-</td>
</tr>
<tr class="h">
<td style="" >Total Unpaid Salary</td>
<td class="r" >'.$unpaid.'/-</td>
</tr>
    ...
    echo'<table style="width:100%;font-family:times;font-weight:bold;text-align:center;" class="F17" >
    <tr style="height:35px;" >
    <td colspan="4" >'.$itemname.'</td>
    </tr>
    <tr class="F13" style="height:35px;">
...
<td style="text-align:center;">'.$hold.'% Hold</td>
<td style="text-align:right;">'.$gamount.'/-</td>
</tr>';
}

echo'<tr>
<td style="text-align:right;" colspan="5" >Total</td>
<td style="text-align:right;" >'.$amt2.'/-</td>
</tr>
</table>';
}
echo '<div class="F14" style="text-align:right;padding-right:3%;" >Total : '.$amt1.'/-</div>
</div>';
